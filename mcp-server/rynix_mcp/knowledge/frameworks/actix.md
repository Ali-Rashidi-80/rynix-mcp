---
name: actix
description: Security testing for Actix Web — guards, apps, scopes, and async handler auth
---

# Actix Web

Security testing for Actix Web — guards, apps, scopes, and async handler auth. Map routes with `analyze_repo` (stack hint `actix`), then validate authorization with live probes — never trust framework defaults.
...
Rynix `analyze_repo` flags routes containing: `HttpAuthentication`, `Identity`, `authorize`, `#[guard]`, `require_auth`.
...
1. **Enumerate** — `analyze_repo` + `list_api_routes` / `list_frontend_routes` for stack `actix`.
...
- Set `stack_hints=["actix"]` in profile `[scanner]` for dedicated extractor routing.
...
2. `analyze_repo` with `stack_hints=["actix"]` — review `detected_stack` confidence.
