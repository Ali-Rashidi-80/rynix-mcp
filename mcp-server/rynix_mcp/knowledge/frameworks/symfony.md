---
name: symfony
description: Security testing for Symfony — voters, firewalls, attributes, and Doctrine
---

# Symfony

Security testing for Symfony — voters, firewalls, attributes, and Doctrine. Map routes with `analyze_repo` (stack hint `symfony`), then validate authorization with live probes — never trust framework defaults.
...
Rynix `analyze_repo` flags routes containing: `#[IsGranted]`, `is_granted`, `security.yaml`, `ROLE_`, `Voter`.
...
1. **Enumerate** — `analyze_repo` + `list_api_routes` / `list_frontend_routes` for stack `symfony`.
...
- Set `stack_hints=["symfony"]` in profile `[scanner]` for dedicated extractor routing.
...
2. `analyze_repo` with `stack_hints=["symfony"]` — review `detected_stack` confidence.
