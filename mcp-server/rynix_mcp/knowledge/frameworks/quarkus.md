---
name: quarkus
description: Security testing for Quarkus — JAX-RS, Panache, OIDC, and reactive routes
---

# Quarkus

Security testing for Quarkus — JAX-RS, Panache, OIDC, and reactive routes. Map routes with `analyze_repo` (stack hint `quarkus`), then validate authorization with live probes — never trust framework defaults.
...
Rynix `analyze_repo` flags routes containing: `@RolesAllowed`, `@Authenticated`, `SecurityIdentity`, `@PermitAll`, `quarkus-oidc`.
...
1. **Enumerate** — `analyze_repo` + `list_api_routes` / `list_frontend_routes` for stack `quarkus`.
...
- Set `stack_hints=["quarkus"]` in profile `[scanner]` for dedicated extractor routing.
...
2. `analyze_repo` with `stack_hints=["quarkus"]` — review `detected_stack` confidence.
