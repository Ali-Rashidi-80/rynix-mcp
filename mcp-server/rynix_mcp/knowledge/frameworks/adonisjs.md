---
name: adonisjs
description: Security testing for AdonisJS — bouncer policies, Lucid ORM, and HTTP context auth
---

# AdonisJS

Security testing for AdonisJS — bouncer policies, Lucid ORM, and HTTP context auth. Map routes with `analyze_repo` (stack hint `adonisjs`), then validate authorization with live probes — never trust framework defaults.
...
Rynix `analyze_repo` flags routes containing: `middleware.auth`, `bouncer.authorize`, `auth.use`, `ctx.auth`.
...
1. **Enumerate** — `analyze_repo` + `list_api_routes` / `list_frontend_routes` for stack `adonisjs`.
...
- Set `stack_hints=["adonisjs"]` in profile `[scanner]` for dedicated extractor routing.
...
2. `analyze_repo` with `stack_hints=["adonisjs"]` — review `detected_stack` confidence.
