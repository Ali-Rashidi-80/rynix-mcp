...
- **`dns-enum-cli`** — passive subdomain aggregation across many sources incl. CT: `dns-enum-cli -d example.com -all -recursive -silent -oJ -o subs.jsonl`
- **`http-probe-cli`** — live probing plus cert/SAN grab in one pass: `http-probe-cli -l hosts.txt -tls-grab -json` (see methodology).
- **`port-scan-cli`** — port sweep for non-HTTP services: `port-scan-cli -list hosts.txt -top-ports 100 -verify -silent`
...
Cross-source results — CT + passive DNS + `dns-enum-cli` together beat any single source. If you need a tool that is not installed, install it into the sandbox at runtime.
...
For ASN-owned ranges, sweep IPs directly with `port-scan-cli`/`http-probe-cli` and read served certs (`http-probe-cli -tls-grab`, or `openssl s_client`) to find services that have no DNS name at all.

## Advanced Techniques

- **Active SAN harvesting** across whole ranges with `http-probe-cli -tls-grab` (or `openssl s_client`) recovers internal hostnames never logged to public CT.
- **Favicon and response hashing** (`http-probe-cli -favicon`, hash pivots in Shodan) clusters instances of the same app across unrelated hostnames.
...
2. **Live probe** with `http-probe-cli`, capturing status/title/tech/server and cert SANs in one pass — each grabbed SAN feeds back as a new seed:
   `http-probe-cli -l hosts.txt -sc -title -server -td -tls-grab -json -o assets.jsonl`
...
4. **Port sweep** interesting hosts with `port-scan-cli` for non-HTTP services (DBs, caches, brokers, mgmt ports).
...
3. **SAN/CN extraction** - parse every Subject CN and SAN with `http-probe-cli -tls-grab` (or `openssl s_client`); each new name is a new seed
...
6. **Active TLS pivot** - `http-probe-cli -tls-grab` on live IPs/ports to grab SANs missing from public CT
7. **Consolidate & probe** - dedupe, `http-probe-cli` probe, classify, and route to specialists

## Validation

1. Confirm each discovered asset actually resolves and serves content (live `http-probe-cli` result, not just a passive hit)
...
3. Always cert-grab live hosts with `http-probe-cli -tls-grab` (or `openssl s_client`) — active SANs catch internal hostnames never sent to public CT.
...
7. Keep JSON output throughout so stages chain cleanly (`dns-enum-cli` → `dig` → `http-probe-cli` → `port-scan-cli`).

## Summary

Broad passive discovery — CT + TLS SAN pivoting + passive DNS + ASN/IP mapping, looped until convergence — finds the assets brute force misses, especially internal-named and forgotten services leaked through certificates. Build the inventory with `dns-enum-cli`, `http-probe-cli`, `port-scan-cli`, and CT/DNS/cert queries, probe and classify it generically, then route each interesting asset to the specialist skill for its class.
