...
6. Pair with `ssrf`, `information_disclosure`, `kubernetes`, `aws`/`gcp`, and `authentication_jwt` skills; use `template-scan` templates (`grafana-*`, `prometheus-*`) for fast triage.
