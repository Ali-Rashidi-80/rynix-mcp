---
name: web-crawler
description: web-crawler crawler syntax, depth/js/known-files behavior, and stable concurrency controls.
---

# web-crawler CLI Playbook

Official docs:
- https://docs.rynix.local/docs/opensource/web-crawler/usage
- https://docs.rynix.local/docs/opensource/web-crawler/running
- https://docs.rynix.local/docs/web-crawler
...
`mkdir -p crawl && web-crawler -u https://target.tld -d 3 -ct 10m -mdp 2000 -fsu -jc -kf robotstxt -c 10 -p 10 -rl 50 -timeout 10 -retry 1 -ef png,jpg,jpeg,gif,svg,css,woff,woff2,ttf,eot,map -silent -j -o crawl/web-crawler.jsonl`

Common patterns:
- Fast crawl baseline:
  `web-crawler -u https://target.tld -d 3 -jc -silent`
- Deeper JS-aware crawl (narrowed target; keep it time-bounded):
  `web-crawler -u https://target.tld -d 5 -ct 15m -jc -jsl -kf all -c 10 -p 10 -rl 50 -o katana_urls.txt`
- Multi-target run with JSONL output:
  `web-crawler -list urls.txt -d 3 -jc -silent -j -o web-crawler.jsonl`
- Headless crawl with local Chrome:
  `web-crawler -u https://target.tld -hl -sc -nos -xhr -j -o crawl/katana_headless.jsonl`
- Headless crawl through proxy:
  `web-crawler -u https://target.tld -hl -sc -ho proxy-server=http://127.0.0.1:48080 -j -o crawl/katana_proxy.jsonl`
...
Keeping output small (web-crawler has NO default page cap, so plan for volume):
...
- Reduce, then delete: once the crawl finishes, extract just what you need (e.g. `web-crawler ... -f url -o urls.txt` or `sort -u` a URL list, or a short note of interesting paths) and remove the raw crawl file/dir. Don't keep large raw crawls around after you've distilled them.
...
- Prefer `-proxy` over environment proxy variables when proxying only web-crawler traffic.
...
`site:docs.rynix.local/docs web-crawler <flag> usage`

Complementary crawlers / JS endpoint extractors in the sandbox:
- `gospider -s https://target.tld -d 3 -c 10 -t 20` — alternate crawler;
  picks up things web-crawler misses on weird sites; use it as a second
  pass when web-crawler output looks thin.
- `~/tools/JS-Snooper/js_snooper.sh <domain>` and
  `~/tools/jsniper.sh/jsniper.sh <domain>` — both take a bare domain and
  run their own JS-file discovery internally (jsniper drives http-probe-cli +
  web-crawler + template-scan file templates). Reach for them when you want a quick
