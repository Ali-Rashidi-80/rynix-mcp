...
- HTTP login endpoints: `fuzz-cli` or custom scripts (see Tooling)
...
### fuzz-cli (primary for web logins)

- Login brute-force with multiple users and passwords:
  `fuzz-cli -w users.txt:USER -w passwords.txt:PASS -u https://target.com/login -X POST -d "username=USER&password=PASS" -fr "Invalid"`
...
9. For web logins prefer `fuzz-cli`; for other services use `nmap` NSE `*-brute` scripts or custom scripts with equivalent logic
