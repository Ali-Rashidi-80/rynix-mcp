...
pub fn extract_hono_routes(repo: &Path, config: &ScanConfig) -> Vec<ApiRoute> {
    let roots = source_roots(repo, config);
    let files = walk_files(repo, &roots, &["js", "ts"]);
    let re = Regex::new(
        r#"(?i)(?:app|hono)\s*\.\s*(get|post|put|patch|delete|all)\s*\(\s*['"]([^'"]+)['"]"#,
    )
    .unwrap();
    let mut routes = Vec::new();
    for (_path, rel) in files {
        let content = std::fs::read_to_string(repo.join(&rel)).unwrap_or_default();
        if !content.contains("app.") && !content.contains("Hono") {
            continue;
        }
