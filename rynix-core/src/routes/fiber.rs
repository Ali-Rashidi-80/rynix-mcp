...
pub fn extract_fiber_routes(repo: &Path, config: &ScanConfig) -> Vec<ApiRoute> {
    let roots = source_roots(repo, config);
    let files = walk_files(repo, &roots, &["go"]);
    let re = Regex::new(
        r#"(?:app|fiber)\s*\.\s*(Get|Post|Put|Patch|Delete)\s*\(\s*[\"']([^\"']+)[\"']"#,
    )
    .unwrap();
    let mut routes = Vec::new();
    for (_path, rel) in files {
        let content = std::fs::read_to_string(repo.join(&rel)).unwrap_or_default();
        if !content.contains("fiber.") && !content.contains("app.") {
            continue;
        }
