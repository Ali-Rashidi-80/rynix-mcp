...
pub fn extract_express_routes(repo: &Path, config: &ScanConfig) -> Vec<ApiRoute> {
    let roots = source_roots(repo, config);
    let files = walk_files(repo, &roots, &["js", "ts", "mjs", "cjs"]);
    let re = Regex::new(
        r#"(?i)(?:router|app)\s*\.\s*(get|post|put|patch|delete|all)\s*\(\s*["'`](/[^"'`]*)["'`]"#,
    )
    .unwrap();
